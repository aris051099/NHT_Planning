...
{
  public:  
    Eigen::Matrix4d A_dt = Eigen::Matrix4d::Zero(); //Discrete time A_matrix(dt = 0.01) (From MATLAB)
    Eigen::MatrixXd B_dt = Eigen::MatrixXd::Zero(4,2); //Discrete time B_matrix(dt=0.01) (From MATLAB)
    Eigen::Matrix4d C_dt = Eigen::Matrix4d::Zero(); //Discrete time A_matrix(dt = 0.01) (From MATLAB)
    Eigen::MatrixXd D_dt = Eigen::MatrixXd::Zero(4,2); //Discrete time B_matrix(dt=0.01) (From MATLAB)
    dynamic_system();
};
...
Eigen::Vector4d propagate(Eigen::Vector4d x_k,Eigen::Vector2d u_k, dynamic_system din_sytem,double prop_time)
{
...
float error(Eigen::Vector4d x_goal, Eigen::Vector4d x_near)
{
  float error = 0; 
...
int main()
{
  ...
  double prop_time = 1; //1 second of propagation
  ...
  Eigen::Vector4d x_0(1.9,0.05,0.20,0);
  Eigen::Vector4d x_prop(0,0,0,0);
  Eigen::Vector4d x_goal(2,0.1,0.3,0);
  Eigen::Vector2d u_k(1,1);
